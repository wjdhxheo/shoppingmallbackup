...
public class MngrDBBean {
	...
	public void insertBook(MngrDataBean book) throws Exception {
		...
		try {
			conn = getConnection();
			String sql = "insert into book(book_id, book_kind, book_title, book_price, book_count, author, publishing_com, publishing_date, book_image, book_content, discount_rate, reg_date) ";
	...
	public int registedBookconfirm(String kind, String bookName, String author) throws Exception {
		...
		try {
			conn = getConnection();
			String sql = "select book_title from book where book_kind = ? and book_title = ? and author = ?";
			pstmt = conn.prepareStatement(sql);
			...
			if (rs.next()) { // 해당 책이 이미 존재함
				x = 1;
			} else { // 해당 책이 이미 등록되어있지 않음
				x = -1;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int getBookCount() throws Exception {
		...
		try {
			...
			if (rs.next())
				x = rs.getInt(1); // select 되어오는 전체 책의 카운트수를 변수에 저장한다
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	public String getBookTitle(int book_id) {
		...
		try {
			...
			if (rs.next()) {
				x = rs.getString(1); // select 되어온 book_title 를 변수에저장 (인덱스번호 사용)
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void updateBook(MngrDataBean book, int bookId)throws Exception{
		...
		sql = "update book set book_kind = ?, book_title = ?, book_price = ?, book_count = ?, author = ?, publishing_com = ?, publishing_date = ?";
